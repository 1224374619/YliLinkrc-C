      ...
      <div class="station-foot-content">
        ...
        <div class="station-foot-foot">
          ...
          <div class="station-foot-foot-two">
            <span style="font-size:18px">{{positionIdList.company.address.province+' ' +' '+positionIdList.company.address.city}} {{positionIdList.workAddress.detail}}</span>
            <!-- <span>查看地图</span> -->
          </div>
           <el-amap vid="amap" :plugin="plugin" :events="events" class="amap" style="width:859px;height:188px;margin:20px 0 70px 96px" :center="center" :zoom="zoom">
      ...
      <div class="station-foot-aside">
        <div class="station-foot-aside-nav">
          <div class="station-foot-aside-nav-img">
            <img @click="next(companyIdList.id)" style="width 112px;height:112px;cursor:pointer;border-radius:5px" :src="url" />
          </div>
          <div class="station-foot-aside-nav-article">
            ...
            <div class="company-status">
              <span>
                <i class="el-icon-s-data"></i> {{companyIdList.nature}}
              </span>
            </div>
            <div class="company-num">
              <span>
                <i class="el-icon-coordinate"></i> {{companyIdList.size}}
              </span>
            </div>
          </div>
        </div>
        <div class="station-foot-aside-footer">
          ...
          <div v-if="More" class="aside-footer" v-for="(list,index) in hotpositionList.slice(0,6)" :key="index" @click="nextjoblist(list.id)">
            <div class="company-post">
              <span v-if="list.positionName.length > 10">
                <el-tooltip placement="bottom" effect="light">
                  <div slot="content">{{list.positionName}}</div>
                  <span style="margin:0 0 0 0">
                    {{list.positionName.slice(0, 10)+'...'}}
                  </span>
                </el-tooltip>
              </span>
          ...
          <div v-if="noMore" style="margin-top:20px"><h4>暂无更多职位</h4></div>
        </div>
      </div>
    </div>
  </div>
