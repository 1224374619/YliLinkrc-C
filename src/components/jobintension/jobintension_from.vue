  ...
  <div class="from">
    <div class="fromfirst">
      <div class="fromfirst-one">
        职位类型：
        <span style="color:#373737" v-if="fromData.positionCatalogs">{{fromData.positionCatalogs[0].third}}</span>
        <span v-else></span>
      </div>
      <div class="fromfirst-one">
        企业行业：
        <span style="color:#373737" v-if="fromData.industries">{{fromData.industries[0].secondary}}</span>
        <span v-else></span>
      </div>
      <div class="fromfirst-one">
        求职状态：
        <span style="color:#373737">{{fromData.jobSearchStatus}}</span>
      </div>
    </div>
    <div class="fromsecond">
      <div class="fromfirst-two"> 
        工作城市：
        <span style="color:#373737">{{fromData.cities[0].district}}</span>
      </div>
      <div class="fromfirst-two">
        薪资范围：
        <span style="color:#373737"  v-if="fromData.salaryMin === 35">{{fromData.salaryMin}}k以上</span>
        <span v-else>{{fromData.salaryMin}}k-{{fromData.salaryMax}}k</span>
      </div>
      <div class="fromfirst-two">
        工作类型：
        <span style="color:#373737">{{fromData.jobType}}</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.positionList = positionCatalog.data
